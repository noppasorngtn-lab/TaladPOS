...
> No dedicated Domain/Application test task is listed for this phase (unlike T023/T024/T038/T045/T051) — these are read-only search/aggregation queries, not the pricing/stock/void/uniqueness business rules Constitution Principle III mandates coverage for, consistent with US1's `SearchProductsQuery` (T029) also having no test task.

- [X] T057 [US5] Implement `SearchSalesOrdersQuery` (filters: date range, staff, member, status) in `api/src/TaladPOS.Application/SalesOrders/SearchSalesOrdersQuery.cs` (FR-026). All filters are optional (nullable) — an admin can open the history screen with no filters and see everything, matching this repo's existing list-screen convention (Stock/Promotions also default to unfiltered). Extended `ISalesOrderRepository` with `SearchAsync`.
- [X] T058 [US5] Implement report queries — sales-summary (daily/monthly), best-sellers, sales-by-staff, stock-levels — excluding `Voided` orders (FR-033) in `api/src/TaladPOS.Application/Reports/` (FR-029–FR-032). One query class per report (`SalesSummaryQuery`, `BestSellersQuery`, `SalesByStaffQuery`, `StockLevelsQuery`), each a thin wrapper over `IReportsRepository`/`ReportsRepository` (Infrastructure). `from`/`to` are required for the three date-bounded reports per contracts/reports.md's literal notation (missing either → 422). Sales-summary/best-sellers/sales-by-staff group in-memory after a single date-bounded fetch rather than via SQL date-trunc — acceptable at this project's single-store scale (plan.md Scale/Scope) and keeps the grouping logic database-agnostic. Best-sellers ranks by `TotalAmount` descending (contract didn't specify which of quantity/amount is primary).
- [X] T059 [US5] Implement `GET /sales-orders` (history) in `api/src/TaladPOS.Api/Controllers/SalesOrdersController.cs`, and `GET /reports/sales-summary`, `GET /reports/best-sellers`, `GET /reports/sales-by-staff`, `GET /reports/stock-levels` in `api/src/TaladPOS.Api/Controllers/ReportsController.cs` per contracts/sales-orders.md and contracts/reports.md (depends on T057, T058). Both controllers are Admin-only.
- [X] T060 [P] [US5] Sales history screen: PrimeReact `DataTable` with date/staff/member/status filters in `web/src/app/(app)/sales-history/page.tsx` (FR-026). Added `web/src/app/(app)/sales-history/layout.tsx` (Admin-only, mirrors `stock/layout.tsx`).
- [X] T061 [P] [US5] Reports screen(s): one PrimeReact `DataTable` per report type in `web/src/app/(app)/reports/page.tsx` (FR-029–FR-032). Added `web/src/app/(app)/reports/layout.tsx` (Admin-only). Defaults to the first-of-month through today, computed from **local** date parts (not `toISOString()`, which would shift the date for a UTC+7 user before 07:00 — caught during this phase's verification, not by type-checking).

**Verification**: `dotnet build`/`dotnet format --verify-no-changes`/`dotnet test` all pass (35/35 Domain tests — unchanged, no new Domain/Application tests for this phase per the note above; Application/Api.IntegrationTests unaffected). `npm run build`/`lint`/`tsc --noEmit` all pass. Verified in-browser with a seeded session: `/sales-history` and `/reports` both render their DataTables (header/body padding matches the established pattern) and error banners when the real fetch fails against the unreachable database; with stubbed responses, all five tables (history + 4 reports) render real rows correctly, including a `DateOnly`-typed `period` field rendering as a plain `"2026-09-10"` string rather than a timestamp. Found and fixed a real bug during this check: the error banner on both new screens wasn't cleared after a subsequent successful fetch (stale error text persisted once real data loaded) — added `setError(null)` on the success path in both `refresh` functions. Same as Phases 2–6: the actual search/report data against a live database is unverified — the local PostgreSQL credential mismatch noted since Phase 2 (T014) still blocks running real data through the API.
...
- [X] T062 Run the full `quickstart.md` validation walkthrough end-to-end against the deployed `api/` + `web/`. Resolved a long-standing blocker first: `localhost:5432` was being claimed by two different PostgreSQL servers on this machine (a native Windows `postgresql-x64-18` service and an unrelated `lab-postgres16` Docker container), so every `api/` connection attempt since Phase 2 had silently been hitting the wrong server. Stood up a dedicated `taladpos-postgres` Docker container on port 5433 (scoped to this repo, doesn't touch either pre-existing server) and repointed `appsettings.Development.json` at it. With a real database reachable for the first time, ran all 8 quickstart.md steps: steps 1–7 end-to-end via direct API calls (login, product CRUD incl. Thai-name UTF-8 round-trip, checkout with stock decrement, insufficient-stock 409, member signup/duplicate-422/404-lookup, member-linked credit, sequential promotion-then-member discount math per FR-036, future-dated promotion correctly not applied, void with stock restore and double-void 409, history and all four reports correctly excluding voided orders per FR-033) and again through the real browser UI (login → stock table → sales screen cart/checkout/void → sales-history → reports), then step 8 (reliability) by killing the API mid-cart and confirming the UI shows "Could not reach the server..." with no fake success and, on restart, confirming stock/order-count were unchanged (FR-006/SC-007). This is the first phase where real data flowed through the full stack instead of stubbed responses.
- [X] T063 [P] Add API integration tests covering the contracts in `contracts/*.md` in `api/tests/TaladPOS.Api.IntegrationTests/`. Added `Microsoft.AspNetCore.Mvc.Testing`, a `TaladPosApiFactory : WebApplicationFactory<Program>` (required adding `public partial class Program;` to `Program.cs`) pointed at a dedicated `taladpos_test` database that's reset (`EnsureDeleted`+`Migrate`) once per test run via `IAsyncLifetime`, and one xUnit collection (`ApiCollection`) so all test classes share that reset instead of paying for it per-class. 25 tests across `ProductsTests`, `SalesOrdersTests` (checkout/stock/void/pricing), `MembersTests`, `ReportsTests`, and `SecurityTests`, all passing against the real database.
- [X] T064 Security hardening: verify Admin-only endpoints reject a `Cashier`-role JWT with `403`, and verify an expired/invalid JWT is rejected with `401`. Covered by `SecurityTests.cs` (part of T063's suite): Admin-only product-create/low-stock/reports endpoints correctly 403 a Cashier-role token; a Cashier can still reach any-authenticated-staff endpoints (`GET /products`); missing token, a garbage/malformed token, and wrong login credentials all correctly return 401.
- [X] T065 [P] Update the repository root `README.md` with `api/`/`web/` setup and run instructions, referencing `quickstart.md`. Created (none existed before) — covers prerequisites, DB migration, running `api/`/`web/`, running tests, and points to `quickstart.md` for the full walkthrough.
